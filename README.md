...
Uses [spaCy](https://spacy.io/) with the `en_core_web_lg` model. spaCy was chosen because it handles NER out of the box without writing custom regex patterns, and `en_core_web_lg` is small and fast enough for short documents like this one. A pure regex approach would have been brittle for company and person names; spaCy's statistical model handles natural variation better.
(tried using `en_core_web_sm` and `en_core_we_md` but both were giving less accurate results than the larger model)

spaCy and the model were still labelling certain words like 'CFO' as a company so the only way to omit that was to hardcode a list of possibly words that the model may consider an `Org`.
...
- **Model accuracy is imperfect.** `en_core_web_lg` is a general-purpose model. It may miss entities, mis-label them, or pick up false positives — especially for short or ambiguous names.
